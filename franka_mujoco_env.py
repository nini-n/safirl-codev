...
try:
    ...
    raise RuntimeError("mujoco paketi yüklü değil. `pip install mujoco` ile kur.") from e
...
def resolve_asset_path() -> str:
    ...
    cand2 = os.path.normpath(os.path.join(os.getcwd(), "assets", "franka", "franka.xml"))
...
class FrankaMujocoEnv(gym.Env):
    ...
    def __init__(
        self, d_min: float = 0.08, qdot_max: float = 1.2, episode_len: int = 256, seed: int = 0
    ):
        super().__init__()
        ...
        # Eklem indexleri
        self.qpos_idx = [self.model.joint(name).qposadr[0] for name in ["j1", "j2", "j3"]]
        self.qvel_idx = [self.model.joint(name).dofadr[0] for name in ["j1", "j2", "j3"]]
    ...
    def _get_obs(self) -> np.ndarray:
        return np.concatenate([self._get_q(), self._get_qd(), self.goal, self.obs]).astype(
            np.float32
        )
