...
class MPCShield:
    ...
    def __init__(self, d_min=0.08, alpha=2.0, qdot_max=1.2, dt=0.02, horizon=10, rho=0.0):
    ...
    def project(self, state, u_policy):
        ...
        prob = osqp.OSQP()
        prob.setup(P=P, q=qv, A=A, l=l, u=u, verbose=False, polish=False)
        ...
        return np.clip(u0.astype(np.float64), -self.qdot_max, self.qdot_max).astype(np.float32)
